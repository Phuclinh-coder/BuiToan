...
namespace QL_BanGiay
{
    public partial class FrmChitietHDB : Form
    {
        ...
        public FrmChitietHDB()
        {
            ...
            data.FillComboBox(cboMaGiay, dtGiay, "MaGiay", "MaGiay");
        }
        ...
        private void inHoaDon_Click(object sender, EventArgs e)
        {
            ...
            string sql = "select tGiay.MaGiay,TenGiay,SLBan,DonGiaBan,KhuyenMai,(SLBan*(DonGiaBan*(1-KhuyenMai/100))) as ThanhTien \r\n from tChiTietHDB\r\n join tGiay on tChiTietHDB.MaGiay=tGiay.MaGiay ";
         ...
         string tinhTong()
        {
            string sql1 = "  select sum(SLBan*DonGiaBan) as ThanhTien,tChiTietHDB.SoHDB" +
               " from tChiTietHDB join tGiay on tChiTietHDB.MaGiay=tGiay.MaGiay  where SoHDB=N'" + maHDB.Text + "' group by tChiTietHDB.SoHDB";
        ...
        private void cboMaGiay_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                DataTable dtG = data.ReadData("Select * from tGiay where MaGiay='" + cboMaGiay.SelectedValue + "'");
                txtTenGiay.Text = dtG.Rows[0]["TenGiay"].ToString();
        ...
        private void select()
        {
            string sql = "select tGiay.MaGiay,TenGiay,SLBan,DonGiaBan,KhuyenMai,(SLBan*(DonGiaBan-DonGiaBan*(KhuyenMai/100))) as ThanhTien \r\n from tChiTietHDB\r\n join tGiay on tChiTietHDB.MaGiay=tGiay.MaGiay ";
            sql += "where SoHDB=N'" + maHDB.Text + "'";
            danhSachBan.DataSource = data.ReadData(sql);
        }
        public int slban() {
            string sql = "select sum(SLBan) as SLBan from tChiTietHDB where tChiTietHDB.MaGiay=N'" + cboMaGiay.Text + "'\r\ngroup by tChiTietHDB.MaGiay";
            DataTable dt = data.ReadData(sql);
            return int.Parse(dt.Rows[0]["SLBan"].ToString());
        }
        public int sl()
        {
            string sql = "select SoLuong  from tGiay where tGiay.MaGiay=N'" + cboMaGiay.Text + "'";
            DataTable dt = data.ReadData(sql);
        ...
        private void them_Click(object sender, EventArgs e)
        {
            ...
            string sql = " INSERT INTO tChiTietHDB(SoHDB,MaGiay,SLBan,KhuyenMai) VALUES(" + "N'" + maHDB.Text + "','" + cboMaGiay.Text + "','" + int.Parse(txtSoLuong.Text) + "','" + double.Parse(txtGiamGia.Text) + "') ";
            sql += " Update tGiay Set";
            sql += " SoLuong='" + (sl() - int.Parse(txtSoLuong.Text)) + "' ";
            sql += " where MaGiay='" + cboMaGiay.Text + "'";
            data.UpdateData(sql);
            select();
            tinhTong();
            resetvalue();
        ...
        private void danhSachBan_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            xoa.Enabled = true;
            sua.Enabled = true;
            cboMaGiay.Text = danhSachBan.CurrentRow.Cells[0].Value.ToString();
            txtTenGiay.Text = danhSachBan.CurrentRow.Cells[1].Value.ToString();
        ...
        private void sua_Click(object sender, EventArgs e)
        {
            ...
            sql += " where SoHDB=N'" + maHDB.Text.Trim() + "' and MaGiay='"+cboMaGiay.Text+"'";
            data.UpdateData(sql);
            tinhTong();
            select();
        }
        ...
        {
            string sql;           
            if (MessageBox.Show("Bạn có muốn xóa sản phẩm " + txtTenGiay.Text + "?", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                sql = "delete tChiTietHDB where MaGiay=N'" + cboMaGiay.Text + "' and SoHDB=N'"+maHDB.Text+"'";
                data.UpdateData(sql);
