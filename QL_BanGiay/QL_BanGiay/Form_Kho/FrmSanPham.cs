...
namespace QL_BanGiay
{
    public partial class FrmSanPham : Form
    {
        ...
        {
            string masp;
            string sql="select top(5) tGiay.MaGiay from tGiay join tChitietHDB on tgiay.magiay=tchitiethdb.magiay group by tGiay.MaGiay order by SUM(SLBAN) DESC";
        ...
        {
            string sql = "select MaGiay,TenGiay,TenTheLoai,TenThuongHieu,DonGiaNhap,DonGiaBan," +
                "SoLuong,KichThuoc,GioiTinh,MauSac,TenChatLieu " +
                "from tGiay join tThuongHieu on tGiay.MaThuongHieu=tThuongHieu.MaThuongHieu " +
                "join tTheLoai on tTheLoai.MaTheLoai=tGiay.MaTheLoai " +
                "join tChatLieu on tChatLieu.MaChatLieu=tGiay.MaChatLieu";
            dgvSanPham.DataSource = data.ReadData(sql);
            dgvSanPham.Columns[0].HeaderText = "Mã sản phẩm";
            dgvSanPham.Columns[1].HeaderText = "Tên sản phẩm";
            dgvSanPham.Columns[2].HeaderText = "Kiểu giầy";
        ...
        {
            /*
            foreach(Control c in grbChucnang.Controls)
            {
                if(c is System.Windows.Forms.Button)
                {
                    c.Width = (int)(grbChucnang.Width / 4);
                }    
            }*/
        ...
        private void dgvSanPham_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string masp = dgvSanPham.CurrentRow.Cells["MaGiay"].Value.ToString();
            //if (!checkExistForm("Chitietsp"))
            //{
            //    Chitietsp chitietsp = new Chitietsp(masp);
            //    chitietsp.MdiParent = this;
            //    chitietsp.Show();
            //    chitietsp.Dock = DockStyle.Fill;
            //}
            //else ActiveChildForm("Chitietsp");
        ...
        private void btnTimkiem_Click(object sender, EventArgs e)
        {
            ...
            string sql = "select * from tGiay where 1=1";
            if (txtTimkiem.Text!= "Nhập mã sản phẩm hoặc tên sản phẩm")
            {
                sql += " and ( MaGiay like N'%"+txtTimkiem.Text+"%' or TenGiay like N'%"+txtTimkiem.Text+"%')";
            }
            ...
            if (cbKieuGiay.Text != "")
            {
                sql += " and MaTheLoai like N'%" + cbKieuGiay.SelectedValue + "%'";
            }
            if(cbTamgia.Text!="")
            {
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            shopName.Value = "CỬA HÀNG GIÀY BING CHILING";
            //
            ...
            exSheet.get_Range("D10").Value = "Kiểu giầy";
        ...
        public System.Windows.Forms.ListBox SanPhamNoiBat()
        {
            ...
            sql = "select top(5) tGiay.MaGiay,TenGiay,SUM(SLBan) from tGiay join tChitietHDB on tgiay.magiay=tchitiethdb.magiay group by tGiay.MaGiay,TenGiay order by SUM(SLBAN) DESC";
